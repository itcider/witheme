...
if ( ! function_exists( 'wi_get_defaults' ) ) {
	/**
	 * Set default options
	 *
	 * @since 0.1
	 */
	function wi_get_defaults() {
		return apply_filters(
			...
			array(
				...
				'global_colors' => array(
					array(
						'name' => __( 'Contrast', 'witheme' ),
						'slug' => 'contrast',
						'color' => '#222222',
					),
					array(
						/* translators: Contrast number */
						'name' => sprintf( __( 'Contrast %s', 'witheme' ), '2' ),
						'slug' => 'contrast-2',
						'color' => '#575760',
					),
					array(
						/* translators: Contrast number */
						'name' => sprintf( __( 'Contrast %s', 'witheme' ), '3' ),
						'slug' => 'contrast-3',
						'color' => '#b2b2be',
					),
					array(
						'name' => __( 'Base', 'witheme' ),
						'slug' => 'base',
						'color' => '#f0f0f0',
					),
					array(
						/* translators: Base number */
						'name' => sprintf( __( 'Base %s', 'witheme' ), '2' ),
						'slug' => 'base-2',
						'color' => '#f7f8f9',
					),
					array(
						/* translators: Base number */
						'name' => sprintf( __( 'Base %s', 'witheme' ), '3' ),
						'slug' => 'base-3',
						'color' => '#ffffff',
					),
					array(
						'name' => __( 'Accent', 'witheme' ),
						'slug' => 'accent',
						'color' => '#1e73be',
					),
				),
...
if ( ! function_exists( 'wi_get_color_defaults' ) ) {
	/**
	 * Set default options
	 */
	function wi_get_color_defaults() {
		return apply_filters(
			'wi_color_option_defaults',
			array(
				...
				'navigation_text_hover_color' => 'var(--contrast)',
				...
				'back_to_top_background_color' => '#fff',
				'back_to_top_background_color_hover' => '#f1f1f1',
...
if ( ! function_exists( 'wi_get_default_fonts' ) ) {
	 ...
	 */
	function wi_get_default_fonts( $filter = true ) {
		$defaults = array(
		...
		if ( $filter ) {
			return apply_filters( 'wi_font_option_defaults', $defaults );
		}
...
if ( ! function_exists( 'wi_spacing_get_defaults' ) ) {
	 ...
	 * @return array Option defaults.
	 */
	function wi_spacing_get_defaults( $filter = true ) {
		$defaults = array(
			'top_bar_top' => '10',
			'top_bar_right' => '40',
			'top_bar_bottom' => '10',
		...
		if ( $filter ) {
			return apply_filters( 'wi_spacing_option_defaults', $defaults );
		}
...
if ( ! function_exists( 'wi_typography_default_fonts' ) ) {
	/**
	 * Set the default system fonts.
	 *
	 * @since 1.3.40
	 */
	function wi_typography_default_fonts() {
		$fonts = array(
			'inherit',
			'System Stack',
		...
		return apply_filters( 'wi_typography_default_fonts', $fonts );
	}
}
