...
if ( ! function_exists( 'wi_base_css' ) ) {
	/**
	 * wi the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function wi_base_css() {
		...
		$nav_drop_point = wi_get_option( 'nav_drop_point' );
		$nav_location = wi_get_navigation_location();

		if ( ( 'nav-float-right' === $nav_location || 'nav-float-left' === $nav_location ) && $nav_drop_point ) {
			$media_query = sprintf(
				'(max-width: %1$s) and %2$s',
				absint( $nav_drop_point ) . 'px',
				apply_filters( 'wi_not_mobile_menu_media_query', '(min-width: 769px)' )
			);
			...
			$css->start_media_query( $media_query );
				...
				if ( 'nav-float-left' === wi_get_option( 'nav_position_setting' ) ) {
		...
		if ( wi_get_option( 'logo_width' ) ) {
			$css->set_selector( '.site-header .header-image' );
			$css->add_property( 'width', absint( wi_get_option( 'logo_width' ) ), false, 'px' );
		}
	
		if ( wi_get_option( 'back_to_top' ) ) {
			$css->set_selector( '.wi-back-to-top' );
			...
			//$css->add_property( 'opacity', '1' ); // Can't be 0 or we face double-tap issues on iOS.
			//$css->add_property( 'transform', 'translateY(0)' ); // Can't use visibility or we face the same issue as above.
			$css->set_selector( '.wi-back-to-top__show' );
		...
		do_action( 'wi_base_css', $css );

		return apply_filters( 'wi_base_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'wi_advanced_css' ) ) {
	/**
	 * wi the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function wi_advanced_css() {
		$settings = wp_parse_args(
			get_option( 'wi_settings', array() ),
		...
		$css->set_selector( 'a.wi-back-to-top' );
		...
		$css->set_selector( 'a.wi-back-to-top:hover,a.wi-back-to-top:focus' );
		...
		do_action( 'wi_colors_css', $css );

		return apply_filters( 'wi_colors_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'wi_font_css' ) ) {
	/**
	 * wi the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function wi_font_css() {
		...
		$defaults = wi_get_default_fonts( false );

		$css = new witheme_CSS();

		$body_family = wi_get_font_family_css( 'font_body', 'wi_settings', wi_get_default_fonts() );
		$top_bar_family = wi_get_font_family_css( 'font_top_bar', 'wi_settings', wi_get_default_fonts() );
		$site_title_family = wi_get_font_family_css( 'font_site_title', 'wi_settings', wi_get_default_fonts() );
		$site_tagline_family = wi_get_font_family_css( 'font_site_tagline', 'wi_settings', wi_get_default_fonts() );
		$navigation_family = wi_get_font_family_css( 'font_navigation', 'wi_settings', wi_get_default_fonts() );
		$widget_family = wi_get_font_family_css( 'font_widget_title', 'wi_settings', wi_get_default_fonts() );
		...
		$footer_family = wi_get_font_family_css( 'font_footer', 'wi_settings', wi_get_default_fonts() );
		...
		if ( apply_filters( 'wi_do_wp_block_margin_bottom', true ) ) {
		...
		$css->start_media_query( wi_get_media_query( 'mobile' ) );
		$css->set_selector( '.main-title' );
		...
		do_action( 'wi_typography_css', $css );

		return apply_filters( 'wi_typography_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'wi_spacing_css' ) ) {
	/**
	 * Write our dynamic CSS.
	 *
	 * @since 0.1
	 */
	function wi_spacing_css() {
		$settings = wp_parse_args(
			get_option( 'wi_spacing_settings', array() ),
			wi_spacing_get_defaults()
		);

		$defaults = wi_spacing_get_defaults( false );
		$sidebar_layout = wi_get_layout();

		$css = new witheme_CSS();

		$css->set_selector( '.inside-top-bar' );
		$css->add_property( 'padding', wi_padding_css( $settings['top_bar_top'], $settings['top_bar_right'], $settings['top_bar_bottom'], $settings['top_bar_left'] ), wi_padding_css( $defaults['top_bar_top'], $defaults['top_bar_right'], $defaults['top_bar_bottom'], $defaults['top_bar_left'] ) );

		if ( wi_is_using_flexbox() && 'boxes' === wi_get_option( 'container_alignment' ) ) {
			$top_bar_padding = absint( $settings['top_bar_right'] ) + absint( $settings['top_bar_left'] );

			$css->set_selector( '.inside-top-bar.grid-container' );
			$css->add_property( 'max-width', wi_get_option( 'container_width' ) + $top_bar_padding, false, 'px' );
		}

		$css->set_selector( '.inside-header' );
		$css->add_property( 'padding', wi_padding_css( $settings['header_top'], $settings['header_right'], $settings['header_bottom'], $settings['header_left'] ), wi_padding_css( $defaults['header_top'], $defaults['header_right'], $defaults['header_bottom'], $defaults['header_left'] ) );

		if ( wi_is_using_flexbox() ) {
			if ( 'boxes' === wi_get_option( 'container_alignment' ) ) {
				$header_padding = absint( $settings['header_right'] ) + absint( $settings['header_left'] );

				$css->set_selector( '.inside-header.grid-container' );
				$css->add_property( 'max-width', wi_get_option( 'container_width' ) + $header_padding, false, 'px' );
			}
		...
		$css->add_property( 'padding', wi_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ), wi_padding_css( $defaults['content_top'], $defaults['content_right'], $defaults['content_bottom'], $defaults['content_left'] ) );

		if ( apply_filters( 'wi_do_group_inner_container_style', true ) ) {
			$css->set_selector( '.site-main .wp-block-group__inner-container' );
			$css->add_property( 'padding', wi_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ) );
		}
		...
		if ( ! wi_is_using_flexbox() && 'text' === wi_get_option( 'container_alignment' ) ) {
			...
			$css->add_property( 'max-width', wi_get_option( 'container_width' ) + $content_padding, false, 'px' );
		}
		...
		$css->add_property( 'margin', wi_padding_css( '0', $settings['content_right'], '0', $settings['content_left'] ), wi_padding_css( '0', $defaults['content_right'], '0', $defaults['content_left'] ) );
		...
		$css->set_selector( '.main-navigation .main-nav ul ul li a' );
		$css->add_property( 'padding', wi_padding_css( $settings['sub_menu_item_height'], $settings['menu_item'], $settings['sub_menu_item_height'], $settings['menu_item'] ), wi_padding_css( $defaults['sub_menu_item_height'], $defaults['menu_item'], $defaults['sub_menu_item_height'], $defaults['menu_item'] ) );
		...
		$css->add_property( 'padding', wi_padding_css( $settings['widget_top'], $settings['widget_right'], $settings['widget_bottom'], $settings['widget_left'] ), wi_padding_css( $defaults['widget_top'], $defaults['widget_right'], $defaults['widget_bottom'], $defaults['widget_left'] ) );

		if ( wi_is_using_flexbox() ) {
			$css->set_selector( '.footer-widgets-container' );
			$css->add_property( 'padding', wi_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), wi_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
			...
			if ( 'boxes' === wi_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', wi_get_option( 'container_width' ) + $footer_widgets_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.footer-widgets' );
			$css->add_property( 'padding', wi_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), wi_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
		...
		$css->add_property( 'padding', wi_padding_css( '0', '0', '0', $settings['footer_widget_separator'] ), wi_padding_css( '0', '0', '0', $defaults['footer_widget_separator'] ) );
		...
		if ( wi_is_using_flexbox() ) {
			...
			$css->add_property( 'padding', wi_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), wi_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
			...
			if ( 'boxes' === wi_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', wi_get_option( 'container_width' ) + $site_info_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.site-info' );
			$css->add_property( 'padding', wi_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), wi_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
		...
		$css->start_media_query( wi_get_media_query( 'mobile' ) );
		$css->set_selector( '.separate-containers .inside-article, .separate-containers .comments-area, .separate-containers .page-header, .separate-containers .paging-navigation, .one-container .site-content, .inside-page-header' );
		$css->add_property( 'padding', wi_padding_css( $settings['mobile_content_top'], $settings['mobile_content_right'], $settings['mobile_content_bottom'], $settings['mobile_content_left'] ) );
		...
		if ( ! wi_is_using_flexbox() ) {
			...
			$sidebar = strtoupper( $wi_settings['sidebar_widget_background_color'] );
			$content = strtoupper( $wi_settings['content_background_color'] );
		...
		do_action( 'wi_spacing_css', $css );

		return apply_filters( 'wi_spacing_css_output', $css->css_output() . $output );
	}
}

/**
 * wis any CSS that can't be cached (can change from page to page).
 *
 * @since 2.0
 */
function wi_no_cache_dynamic_css() {
	$css = new witheme_CSS();

	if ( wi_is_using_flexbox() ) {
		$right_sidebar_width = apply_filters( 'wi_right_sidebar_width', '30' );
		$left_sidebar_width = apply_filters( 'wi_left_sidebar_width', '30' );
 ...
 * @since 3.0.0
 */
function wi_get_dynamic_css() {
	if ( wi_is_using_dynamic_typography() ) {
		$typography_css = witheme_Typography::get_css();
	} else {
		$typography_css = wi_font_css();
	}

	$css = wi_base_css() . $typography_css . wi_advanced_css() . wi_spacing_css();

	return apply_filters( 'wi_dynamic_css', $css );
}

add_action( 'wp_enqueue_scripts', 'wi_enqueue_dynamic_css', 50 );
/**
 * Enqueue our dynamic CSS.
 *
 * @since 2.0
 */
function wi_enqueue_dynamic_css() {
	if ( apply_filters( 'wi_using_dynamic_css_external_file', false ) ) {
		$css = '';
	} elseif ( ! get_option( 'wi_dynamic_css_output', false ) || is_customize_preview() || apply_filters( 'wi_dynamic_css_skip_cache', false ) ) {
		$css = wi_get_dynamic_css();
	} else {
		$css = get_option( 'wi_dynamic_css_output' ) . '/* End cached CSS */';
	}

	$css = $css . wi_no_cache_dynamic_css();

	wp_add_inline_style( 'wi-style', wp_strip_all_tags( $css ) );
}

add_action( 'init', 'wi_set_dynamic_css_cache' );
/**
 ...
 * @since 2.0
 */
function wi_set_dynamic_css_cache() {
	if ( apply_filters( 'wi_dynamic_css_skip_cache', false ) ) {
		return;
	}

	$cached_css = get_option( 'wi_dynamic_css_output', false );
	$cached_version = get_option( 'wi_dynamic_css_cached_version', '' );

	if ( ! $cached_css || wi_VERSION !== $cached_version ) {
		$css = wi_get_dynamic_css();

		update_option( 'wi_dynamic_css_output', wp_strip_all_tags( $css ) );
		update_option( 'wi_dynamic_css_cached_version', esc_html( wi_VERSION ) );
	}
}

add_action( 'customize_save_after', 'wi_update_dynamic_css_cache' );
/**
 * Update our CSS cache when done saving Customizer options.
 *
 * @since 2.0
 */
function wi_update_dynamic_css_cache() {
	if ( apply_filters( 'wi_dynamic_css_skip_cache', false ) ) {
		return;
	}
...
add_action( 'wi_base_css', 'wi_do_modal_css' );
/**
 * Do the modal CSS.
 *
 * @param Object $css The existing CSS object.
 */
function wi_do_modal_css( $css ) {
