...
class witheme_Theme_Update {
	...
	public static function init() {
		...
		$saved_version = get_option( 'wi_db_version', false );
		...
		if ( false === $saved_version ) {
			...
			$existing_settings = get_option( 'wi_settings', array() );
			...
			if ( ! empty( $existing_settings ) ) {
				...
				if ( 'admin_init' === current_action() ) {
					...
					update_option( 'wi_db_version', wi_VERSION );
				}
		...
		if ( version_compare( $saved_version, wi_VERSION, '=' ) ) {
			return;
		}
		...
		// Reset our dynamic CSS file updated time so it rewis.
		$dynamic_css_data = get_option( 'witheme_dynamic_css_data', array() );
		...
		if ( ! empty( $dynamic_css_data ) ) {
			...
			update_option( 'witheme_dynamic_css_data', $dynamic_css_data );
		}
	...
	public static function v_1_3_0() {
		...
		if ( $needs_update ) {
			update_option( 'wi_settings', $settings );
		}
	}
	...
	public static function v_3_0_0() {
		...
		$spacing_settings = get_option( 'wi_spacing_settings', array() );
		...
		if ( $update_spacing_options ) {
			update_option( 'wi_spacing_settings', $spacing_settings );
		}
...
witheme_Theme_Update::get_instance();
