...
add_action( 'after_setup_theme', 'wi_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 * @since 1.3.47
 */
function wi_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'wi_construct_sidebars' );
}
 ...
 * @since 3.2.0
 */
function wi_get_woocommerce_wrapper_tagname() {
	echo is_singular()
		? 'article'
		: 'div';
}

if ( ! function_exists( 'wi_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'wi_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 * @since 1.3.22
	 */
	function wi_woocommerce_start() {
		?>
		<div <?php wi_do_attr( 'content' ); ?>>
			<main <?php wi_do_attr( 'main' ); ?>>
				...
				?>
				<<?php wi_get_woocommerce_wrapper_tagname(); ?> <?php wi_do_attr( 'woocommerce-content' ); ?>>
...
if ( ! function_exists( 'wi_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'wi_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 * @since 1.3.22
	 */
	function wi_woocommerce_end() {
		?>
						</div>
						<?php
						/**
				...
				</<?php wi_get_woocommerce_wrapper_tagname(); ?>>
				<?php
				/**
...
if ( ! function_exists( 'wi_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'wi_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 * @since 1.3.45
	 */
	function wi_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}
...
if ( ! function_exists( 'wi_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'wi_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 * @since 1.3.45
	 */
	function wi_bbpress_css() {
...
if ( ! function_exists( 'wi_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'wi_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 * @since 1.3.45
	 */
	function wi_buddypress_css() {
...
if ( ! function_exists( 'wi_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'wi_beaver_builder_css', 100 );
	/**
	 ...
	 * @since 1.3.45
	 */
	function wi_beaver_builder_css() {
		...
		if ( in_array( 'fl-builder', $body_classes ) && ! in_array( 'full-width-content', $body_classes ) && ! in_array( 'contained-content', $body_classes ) ) {
			...
			if ( $post_date < $compare_date ) {
				...
				$css = str_replace( array( "\r", "\n", "\t" ), '', $css );
				wp_add_inline_style( 'wi-style', $css );
			}
		}
	}
}

add_action( 'wp_enqueue_scripts', 'wi_do_third_party_plugin_css', 50 );
/**
 * Add CSS for third-party plugins.
 *
 * @since 3.0.1
 */
function wi_do_third_party_plugin_css() {
	$css = new witheme_CSS();

	if ( wi_is_using_flexbox() && class_exists( 'Elementor\Plugin' ) ) {
		$css->set_selector( '.elementor-template-full-width .site-content' );
		$css->add_property( 'display', 'block' );
	}

	if ( $css->css_output() ) {
		wp_add_inline_style( 'wi-style', $css->css_output() );
	}
}

add_action( 'wp_enqueue_scripts', 'wi_do_pro_compatibility', 50 );
/**
 * Add CSS to ensure compatibility with GP Premium.
 *
 * @since 3.0.0
 */
function wi_do_pro_compatibility() {
	...
	if ( version_compare( GP_PREMIUM_VERSION, '1.11.0-alpha.1', '<' ) ) {
		if ( wi_is_using_flexbox() && defined( 'wi_SECONDARY_NAV_VERSION' ) ) {
	...
	if ( version_compare( GP_PREMIUM_VERSION, '1.12.0-alpha.1', '<' ) ) {
		if ( defined( 'wi_MENU_PLUS_VERSION' ) && function_exists( 'wi_menu_plus_get_defaults' ) ) {
			$menu_plus_settings = wp_parse_args(
				get_option( 'wi_menu_plus_settings', array() ),
				wi_menu_plus_get_defaults()
			);

			if ( wi_is_using_flexbox() && ( 'true' === $menu_plus_settings['sticky_menu'] || 'desktop' === $menu_plus_settings['sticky_menu'] || 'mobile' === $menu_plus_settings['sticky_menu'] || 'enable' === $menu_plus_settings['mobile_header_sticky'] ) ) {
				if ( wi_has_inline_mobile_toggle() ) {
					$css->start_media_query( wi_get_media_query( 'mobile-menu' ) );
				...
				if ( ! $menu_plus_settings['navigation_as_header'] ) {
					...
					if ( function_exists( 'wi_spacing_get_defaults' ) ) {
					...
					if ( function_exists( 'wi_is_using_flexbox' ) && wi_is_using_flexbox() ) {
						if ( function_exists( 'wi_get_option' ) && 'text' === wi_get_option( 'container_alignment' ) ) {
			...
			if ( wi_has_inline_mobile_toggle() && 'false' !== $menu_plus_settings['slideout_menu'] ) {
			...
			if ( 'font' === wi_get_option( 'icons' ) ) {
				...
				$css->add_property( 'font-family', 'witheme' );
			...
			if ( wi_is_using_flexbox() && $menu_plus_settings['navigation_as_header'] ) {
				$content_left = 40;
				$content_right = 40;
				...
				if ( 'text' === wi_get_option( 'container_alignment' ) ) {
					...
					$css->add_property( 'padding', wi_padding_css( 0, $content_right, 0, $content_left ) );
				}
		...
		if ( defined( 'wi_SECONDARY_NAV_VERSION' ) ) {
			if ( wi_is_using_flexbox() && has_nav_menu( 'secondary' ) ) {
				if ( 'text' === wi_get_option( 'container_alignment' ) && function_exists( 'wi_secondary_nav_get_defaults' ) ) {
					...
					$css->add_property( 'padding', wi_padding_css( 0, $navigation_right_padding, 0, $navigation_left_padding ) );
				}
			}
		}

		if ( wi_is_using_flexbox() && defined( 'wi_FONT_VERSION' ) && function_exists( 'wi_get_default_fonts' ) ) {
			...
			if ( isset( $font_settings['tablet_navigation_font_size'] ) && '' !== $font_settings['tablet_navigation_font_size'] ) {
				$css->start_media_query( wi_get_media_query( 'tablet' ) );
		...
		if ( ! wi_show_title() ) {
			...
			if ( is_single() ) {
				if ( ! apply_filters( 'wi_post_author', true ) && ! apply_filters( 'wi_post_date', true ) ) {
...
add_filter( 'wi_menu_item_dropdown_arrow_direction', 'wi_set_pro_menu_item_arrow_directions', 10, 3 );
/**
 ...
 */
function wi_set_pro_menu_item_arrow_directions( $arrow_direction, $args, $depth ) {
	if ( function_exists( 'wi_secondary_nav_get_defaults' ) && 'secondary' === $args->theme_location ) {
...
add_filter( 'wi_menu_plus_option_defaults', 'wi_set_menu_plus_compat_defaults' );
/**
 ...
 */
function wi_set_menu_plus_compat_defaults( $defaults ) {
	if ( wi_has_inline_mobile_toggle() ) {
...
add_filter( 'wi_spacing_option_defaults', 'wi_set_spacing_compat_defaults', 20 );
/**
 ...
 * @param array $defaults The existing defaults.
 */
function wi_set_spacing_compat_defaults( $defaults ) {
...
add_filter( 'wi_page_hero_css_output', 'wi_do_pro_page_hero_css', 10, 2 );
/**
 ...
 */
function wi_do_pro_page_hero_css( $css_output, $options ) {
	if ( ! defined( 'GP_PREMIUM_VERSION' ) ) {
		return $css_output;
	}
	...
	if ( version_compare( GP_PREMIUM_VERSION, '1.12.0-alpha.1', '<' ) ) {
		...
		if ( wi_is_using_flexbox() && 'text' === wi_get_option( 'container_alignment' ) ) {
			$padding_inside = true;
		}
		...
		if ( wi_is_using_flexbox() && '' !== $options['site_header_merge'] ) {
			if ( 'merge-desktop' === $options['site_header_merge'] ) {
				$css->start_media_query( apply_filters( 'wi_not_mobile_media_query', '(min-width: 769px)' ) );
			}
...
add_action( 'customize_register', 'wi_pro_compat_customize_register', 100 );
/**
 ...
 */
function wi_pro_compat_customize_register( $wp_customize ) {
	if ( ! defined( 'GP_PREMIUM_VERSION' ) ) {
		return;
	}

	if ( version_compare( GP_PREMIUM_VERSION, '1.12.0-alpha.1', '<' ) ) {
		if ( $wp_customize->get_setting( 'wi_spacing_settings[separator]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[separator]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[right_sidebar_width]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[right_sidebar_width]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[left_sidebar_width]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[left_sidebar_width]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_top]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_right]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_bottom]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_left]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_widget_container_left]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_top]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_right]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_bottom]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_left]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[mobile_footer_widget_container_left]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_top]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_top]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_right]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_right]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_bottom]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_bottom]' )->transport = 'refresh';
		}

		if ( $wp_customize->get_setting( 'wi_spacing_settings[footer_left]' ) ) {
			$wp_customize->get_setting( 'wi_spacing_settings[footer_left]' )->transport = 'refresh';
		}
	}

	if ( $wp_customize->get_panel( 'wi_typography_panel' ) ) {
		$wp_customize->get_panel( 'wi_typography_panel' )->active_callback = function() {
...
add_action( 'wp', 'wi_do_pro_compatibility_setup' );
/**
 * Do basic compatibility with GP Premium versions.
 *
 * @since 3.0.0
 */
function wi_do_pro_compatibility_setup() {
	if ( ! defined( 'GP_PREMIUM_VERSION' ) ) {
		return;
	}
	...
	if ( version_compare( GP_PREMIUM_VERSION, '1.12.0-alpha.1', '<' ) ) {
		...
		if ( function_exists( 'wi_premium_do_elements' ) && ! is_singular() ) {
			add_filter( 'wi_show_title', '__return_true', 20 );
		}
	}

	if ( wi_is_using_dynamic_typography() ) {
		remove_action( 'wp_enqueue_scripts', 'wi_enqueue_google_fonts', 0 );
		remove_action( 'wp_enqueue_scripts', 'wi_typography_premium_css', 100 );
		remove_filter( 'wi_external_dynamic_css_output', 'wi_typography_add_to_external_stylesheet' );
	}
}

add_filter( 'wi_has_active_menu', 'wi_do_pro_active_menus' );
/**
 ...
 */
function wi_do_pro_active_menus( $has_active_menu ) {
	if ( ! defined( 'GP_PREMIUM_VERSION' ) ) {
		return $has_active_menu;
	}

	if ( version_compare( GP_PREMIUM_VERSION, '2.1.0-alpha.1', '<' ) ) {
		if ( function_exists( 'wi_menu_plus_get_defaults' ) ) {
			$menu_plus_settings = wp_parse_args(
				get_option( 'wi_menu_plus_settings', array() ),
				wi_menu_plus_get_defaults()
			);
		...
		if ( function_exists( 'wi_secondary_nav_get_defaults' ) && has_nav_menu( 'secondary' ) ) {
			$has_active_menu = true;
		}
	}
...
add_action( 'init', 'wi_do_customizer_compatibility_setup' );
/**
 * Make changes to the Customizer in the Pro version.
 */
function wi_do_customizer_compatibility_setup() {
	if ( ! defined( 'GP_PREMIUM_VERSION' ) ) {
		return;
	}
	...
	if ( version_compare( GP_PREMIUM_VERSION, '2.1.0-alpha.1', '<' ) ) {
		if ( wi_is_using_dynamic_typography() ) {
			remove_action( 'customize_register', 'wi_fonts_customize_register' );
			remove_action( 'customize_preview_init', 'wi_typography_customizer_live_preview' );
		}

		remove_action( 'customize_register', 'wi_colors_customize_register' );
		remove_action( 'customize_preview_init', 'wi_colors_customizer_live_preview' );
		remove_action( 'customize_controls_enqueue_scripts', 'wi_enqueue_color_palettes', 1001 );
		remove_action( 'customize_register', 'wi_colors_secondary_nav_customizer', 1000 );
		remove_action( 'customize_register', 'wi_slideout_navigation_color_controls', 150 );
		remove_action( 'customize_register', 'wi_colors_wc_customizer', 100 );
	}
}
