( function( api ) {
	...
	api.controlConstructor['gp-customizer-typography'] = api.Control.extend( {
		ready: function() {
			...
			control.container.on( 'change', '.witheme-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the wi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'wi_get_all_google_fonts_ajax',
						...
						if ( id in fonts ) {
							...
							jQuery( '.witheme-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.witheme-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.witheme-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.witheme-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.witheme-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.witheme-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.witheme-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.witheme-font-variant' );
			...
			control.container.on( 'change', '.witheme-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.witheme-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.witheme-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.witheme-font-family select' ).select2();

	$( '.witheme-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".witheme-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.witheme-font-variant' ).hide();
		}
	} );
