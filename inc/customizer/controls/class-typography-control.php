...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'wi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 * @since 2.0
	 */
	class wi_Typography_Customize_Control extends WP_Customize_Control {
		/**
		 * Set the type.
		 *
		 * @var string $type
		...
		public function to_json() {
			...
			$this->json['default_fonts_title'] = __( 'System fonts', 'witheme' );
			$this->json['google_fonts_title'] = __( 'Google fonts', 'witheme' );
			$this->json['default_fonts'] = wi_typography_default_fonts();
			$this->json['family_title'] = esc_html__( 'Font family', 'witheme' );
			$this->json['weight_title'] = esc_html__( 'Font weight', 'witheme' );
			$this->json['transform_title'] = esc_html__( 'Text transform', 'witheme' );
			$this->json['category_title'] = '';
			$this->json['variant_title'] = esc_html__( 'Variants', 'witheme' );
		...
		public function content_template() {
					...
					<label>
						<select {{{ data.family.link }}} data-category="{{{ data.category.id }}}" data-variants="{{{ data.variant.id }}}" style="width:100%;">
							...
							<optgroup label="{{ data.google_fonts_title }}">
								<# for ( var key in withemeTypography.googleFonts ) { #>
									<option value="{{ withemeTypography.googleFonts[ key ].name }}"  <# if ( withemeTypography.googleFonts[ key ].name === data.family.value ) { #>selected="selected"<# } #>>{{ withemeTypography.googleFonts[ key ].name }}</option>
								<# } #>
							</optgroup>
						</select>
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				var font_data = withemeTypography.googleFonts[id];
				var variants = '';
				...
				#>
				<div id={{{ data.variant.id }}}" class="witheme-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="witheme-font-category">
					<label>
			...
			<div class="witheme-weight-transform-wrapper">
				<# if ( 'undefined' !== typeof ( data.weight ) ) { #>
					<div class="witheme-font-weight">
						<label>
				...
				<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
					<div class="witheme-font-transform">
						<label>
