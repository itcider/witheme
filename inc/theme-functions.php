...
if ( ! function_exists( 'wi_get_layout' ) ) {
	/**
	...
	function wi_get_layout() {
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = wi_get_option( 'blog_layout_setting' );
		}

		return apply_filters( 'wi_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'wi_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function wi_get_footer_widgets() {
		$widgets = wi_get_option( 'footer_widget_setting' );

		if ( is_singular() ) {
			$widgets_meta = get_post_meta( get_the_ID(), '_wi-footer-widget-meta', true );
		...
		return apply_filters( 'wi_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'wi_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 * @since 1.3.15
	 */
	function wi_show_excerpt() {
		global $post;

		// Check to see if the more tag is being used.
		$more_tag = apply_filters( 'wi_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		$show_excerpt = ( 'excerpt' === wi_get_option( 'post_content' ) ) ? true : false;
		...
		return apply_filters( 'wi_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'wi_show_title' ) ) {
	/**
	 ...
	 */
	function wi_show_title() {
		return apply_filters( 'wi_show_title', true );
	}
}
 ...
 * @since 3.0.0
 */
function wi_show_entry_header() {
	$show_entry_header      = true;
	$show_title             = wi_show_title();
	$has_before_entry_title = has_action( 'wi_before_entry_title' );
	$has_after_entry_title  = has_action( 'wi_after_entry_title' );

	if ( is_page() ) {
		$has_before_entry_title = has_action( 'wi_before_page_title' );
		$has_after_entry_title  = has_action( 'wi_after_page_title' );
	}
	...
	return apply_filters( 'wi_show_entry_header', $show_entry_header );
}

if ( ! function_exists( 'wi_get_premium_url' ) ) {
	/**
	 * wi a URL to our premium add-ons.
	 ...
	 * @return string The URL to wi.itcider.com.
	 */
	function wi_get_premium_url( $url = 'https://wi.itcider.com/premium', $trailing_slash = true ) {
		if ( $trailing_slash ) {
...
if ( ! function_exists( 'wi_padding_css' ) ) {
	/**
	 ...
	 */
	function wi_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'wi_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'wi_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function wi_get_navigation_location() {
		return apply_filters( 'wi_navigation_location', wi_get_option( 'nav_position_setting' ) );
	}
}
 ...
 * @since 2.3
 */
function wi_has_logo_site_branding() {
	$has_site_title = ! wi_get_option( 'hide_title' ) && get_bloginfo( 'title' );
	$has_site_tagline = ! wi_get_option( 'hide_tagline' ) && get_bloginfo( 'description' );
 ...
 */
function wi_get_svg_icon( $icon, $replace = false ) {
	if ( 'svg' !== wi_get_option( 'icons' ) ) {
		return;
	}
	...
	$output = apply_filters( 'wi_svg_icon_element', $output, $icon );
	...
	return apply_filters( 'wi_svg_icon', $output, $icon );
}
 ...
 */
function wi_do_svg_icon( $icon, $replace = false ) {
	echo wi_get_svg_icon( $icon, $replace ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function wi_get_media_query( $name ) {
	$desktop = apply_filters( 'wi_desktop_media_query', '(min-width:1025px)' );
	$tablet_only = apply_filters( 'wi_tablet_media_query', '(min-width: 769px) and (max-width: 1024px)' );
	$mobile = apply_filters( 'wi_mobile_media_query', '(max-width:768px)' );
	$mobile_menu = apply_filters( 'wi_mobile_menu_media_query', $mobile );
 ...
 */
function wi_do_element_classes( $context, $class = '' ) {
	$after = apply_filters( 'wi_after_element_class_attribute', '', $context );

	if ( $after ) {
		$after = ' ' . $after;
	}

	echo 'class="' . join( ' ', wi_get_element_classes( $context, $class ) ) . '"' . $after; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function wi_get_element_classes( $context, $class = '' ) {
	$classes = array();
	...
	return apply_filters( "wi_{$context}_class", $classes, $class );
}
 ...
 * @since 3.0.0
 */
function wi_get_schema_type() {
	return apply_filters( 'wi_schema_type', 'microdata' );
}
 ...
 */
function wi_get_microdata( $context ) {
	$data = false;
	...
	if ( 'body' === $context ) {
		...
		$type = apply_filters( 'wi_body_itemtype', $type );
	...
	if ( 'article' === $context ) {
		$type = apply_filters( 'wi_article_itemtype', 'CreativeWork' );
	...
	if ( $data ) {
		return apply_filters( "wi_{$context}_microdata", $data );
	}
}
 ...
 * @param string $context The element to target.
 */
function wi_do_microdata( $context ) {
	echo wi_get_microdata( $context ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 * @since 3.0.0
 */
function wi_is_using_hatom() {
	return apply_filters( 'wi_is_using_hatom', true );
}
 ...
 * @since 3.0.0
 */
function wi_has_menu_bar_items() {
	return has_action( 'wi_menu_bar_items' );
}
 ...
 */
function wi_do_template_part( $template ) {
	/**
	 * wi_before_do_template_part hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'wi_before_do_template_part', $template );

	if ( apply_filters( 'wi_do_template_part', true, $template ) ) {
	...
	/**
	 * wi_after_do_template_parts hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'wi_after_do_template_part', $template );
}
 ...
 * @since 3.0.0
 */
function wi_has_inline_mobile_toggle() {
	$has_inline_mobile_toggle = wi_is_using_flexbox() && ( 'nav-float-right' === wi_get_navigation_location() || 'nav-float-left' === wi_get_navigation_location() );

	return apply_filters( 'wi_has_inline_mobile_toggle', $has_inline_mobile_toggle );
}
 ...
 * @since 3.0.0
 */
function wi_get_the_title_parameters() {
	$params = array(
		'before' => sprintf(
	...
	return apply_filters( 'wi_get_the_title_parameters', $params );
}
 ...
 * @since 3.0.0
 */
function wi_has_default_loop() {
	return apply_filters( 'wi_has_default_loop', true );
}
 ...
 * @since 3.0.0
 */
function wi_needs_site_branding_container() {
	$container = false;

	if ( wi_has_logo_site_branding() ) {
		if ( wi_is_using_flexbox() || wi_get_option( 'inline_logo_site_branding' ) ) {
			$container = true;
		}
	}
 ...
 *
 * The contextual filter is of the form `wi_attr_{context}`.
 *
 * @since 3.1.0
 *
 ...
 */
function wi_parse_attr( $context, $attributes = array(), $settings = array() ) {
	...
	$classes = wi_get_element_classes( $context );
	...
	// Contextual filter.
	return apply_filters( 'wi_parse_attr', $attributes, $context, $settings );
}

/**
 * Build list of attributes into a string and apply contextual filter on string.
 *
 * The contextual filter is of the form `wi_attr_{context}_output`.
 *
 * @since 3.1.0
 *
 * @param string $context    The context, to build filter name.
 ...
 */
function wi_get_attr( $context, $attributes = array(), $settings = array() ) {
	$attributes = wi_parse_attr( $context, $attributes, $settings );
	...
	$output = apply_filters( 'wi_get_attr_output', $output, $attributes, $context, $settings );
 ...
 * @param array  $settings   Optional. Custom data to pass to filter.
 */
function wi_do_attr( $context, $attributes = array(), $settings = array() ) {
	echo wi_get_attr( $context, $attributes, $settings ); // phpcs:ignore -- Escaping done in function.
}
 ...
 * @since 3.1.0
 */
function wi_get_editor_color_palette() {
	$global_colors = wi_get_option( 'global_colors' );
	$editor_palette = array();
	$static_colors = false;

	if ( apply_filters( 'wi_color_palette_use_static_colors', false ) ) {
		$static_colors = true;
	}
 ...
 * @since 3.1.0
 */
function wi_get_system_default_font() {
	return apply_filters( 'wi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
}
 ...
 * @since 3.1.0
 */
function wi_has_active_menu() {
	$has_active_menu = true;

	if ( ! wi_get_navigation_location() ) {
		$has_active_menu = false;
	}

	return apply_filters( 'wi_has_active_menu', $has_active_menu );
}
 ...
 * @since 3.1.0
 */
function wi_is_using_dynamic_typography() {
	return wi_get_option( 'use_dynamic_typography' );
}
