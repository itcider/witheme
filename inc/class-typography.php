...
class witheme_Typography {
	...
	public function __construct() {
		...
		add_filter( 'wi_editor_styles', array( $this, 'add_editor_styles' ) );
	...
	/**
	 * wi our Google Fonts URI.
	 */
	public static function get_google_fonts_uri() {
		$fonts = wi_get_option( 'font_manager' );
		...
		foreach ( $fonts as $font ) {
			...
			$variants = apply_filters( 'wi_google_font_variants', $variants, $font['fontFamily'] );
		...
		if ( ! empty( $data ) ) {
			$font_args = apply_filters(
				...
				array(
					'family' => implode( '|', $data ),
					'subset' => null,
					'display' => wi_get_option( 'google_font_display' ),
				)
			);
	...
	public function enqueue_google_fonts() {
		...
		if ( $google_fonts_uri ) {
			wp_enqueue_style( 'wi-google-fonts', $google_fonts_uri, array(), wi_VERSION );
		}
	}
	...
	public static function get_css( $module = 'core', $specific_selector = '' ) {
		$typography = wi_get_option( 'typography' );
	...
	public static function get_css_selector( $selector ) {
		...
		return apply_filters( 'wi_typography_css_selector', $selector );
	}
	...
	public static function get_font_family( $font_family ) {
		...
		if ( ! empty( $font_family_args['googleFont'] ) && ! empty( $font_family_args['googleFontCategory'] ) ) {
			...
			$font_family = wi_get_system_default_font();
		}
	...
	public function enqueue_editor_scripts() {
		...
		if ( $html_typography ) {
			wp_add_inline_style(
				 ...
				 *
				 * Related: https://github.com/tomusborne/witheme/issues/472
				 */
				'wp-edit-blocks',
				$html_typography
			);
...
witheme_Typography::get_instance();
