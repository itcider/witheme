 ...
 */
class witheme_Typography_Migration {
	/**
	 * Class instance.
	 *
	 * @access private
	...
	public static function get_option_prefixes() {
		...
		if ( function_exists( 'wi_secondary_nav_typography_selectors' ) ) {
			$data[] = array(
		...
		if ( function_exists( 'wi_menu_plus_typography_selectors' ) ) {
			$data[] = array(
		...
		if ( function_exists( 'wi_woocommerce_typography_selectors' ) ) {
			$data[] = array(
	...
	public static function get_mapped_typography_data() {
		$settings = get_option( 'wi_settings', array() );
		...
		foreach ( self::get_option_prefixes() as $key => $data ) {
			...
			foreach ( $legacy_setting_ids as $name => $id ) {
				...
				if ( 'secondary_navigation' === $data['legacy_prefix'] && function_exists( 'wi_secondary_nav_get_defaults' ) ) {
					$secondary_nav_settings = get_option( 'wi_secondary_nav_settings', array() );
					$secondary_nav_defaults = wi_secondary_nav_get_defaults();
	...
	public static function get_mapped_font_data() {
		...
		foreach ( self::get_option_prefixes() as $key => $data ) {
			...
			if ( 'secondary_navigation' === $data['legacy_prefix'] && function_exists( 'wi_secondary_nav_get_defaults' ) ) {
				$settings = get_option( 'wi_secondary_nav_settings', array() );
				$defaults = wi_secondary_nav_get_defaults();
			}
			...
			if ( self::has_saved_value( 'font_' . $data['legacy_prefix'], $settings, $defaults ) ) {
				...
				$local_fonts = wi_typography_default_fonts();
...
witheme_Typography_Migration::get_instance();
