...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'wi_Customize_Width_Slider_Control' ) ) {
	/**
	 * Create our container width slider control
	 *
	 * @deprecated 1.3.47
	 */
	class wi_Customize_Width_Slider_Control extends WP_Customize_Control {
		/**
		 * Render content.
		 */
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'wiLabelControl' ) ) {
	/**
	 * Heading area
	 *
	 * @since 0.1
	 * @depreceted 1.3.41
	 **/
	class wiLabelControl extends WP_Customize_Control { // phpcs:ignore
		/**
		 * Render content.
		 */
		public function render_content() {}
...
if ( ! class_exists( 'wi_Google_Font_Dropdown_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for all google fonts
	 */
	class wi_Google_Font_Dropdown_Custom_Control extends WP_Customize_Control { // phpcs:ignore
		/**
		 * Set type.
		 *
		 * @var $type
		...
		public function enqueue() {
			wp_enqueue_script( 'witheme-customizer-fonts', trailingslashit( get_template_directory_uri() ) . 'inc/js/typography-controls.js', array( 'customize-controls' ), wi_VERSION, true );
			wp_localize_script( 'witheme-customizer-fonts', 'gp_customize', array( 'nonce' => wp_create_nonce( 'gp_customize_nonce' ) ) );
		}
		...
		public function to_json() {
			...
			$this->json['default_fonts_title'] = __( 'Default fonts', 'witheme' );
			$this->json['google_fonts_title'] = __( 'Google fonts', 'witheme' );
			$this->json['description'] = __( 'Font family', 'witheme' );
			$this->json['google_fonts'] = apply_filters( 'wi_typography_customize_list', wi_get_all_google_fonts( $number_of_fonts ) );
			$this->json['default_fonts'] = wi_typography_default_fonts();
		}
...
if ( ! class_exists( 'wi_Select_Control' ) ) {
	/**
	 * A class to create a dropdown for font weight
	 */
	class wi_Select_Control extends WP_Customize_Control { // phpcs:ignore
		/**
		 * Set type.
		 *
		 * @var $type
...
if ( ! class_exists( 'wi_Hidden_Input_Control' ) ) {
	/**
	 * Create our hidden input control
	 */
	class wi_Hidden_Input_Control extends WP_Customize_Control { // phpcs:ignore
		/**
		 * Set type.
		 *
		 * @var $type
...
if ( ! class_exists( 'wi_Font_Weight_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for font weight
	 *
	 * @deprecated since 1.3.40
	 */
	class wi_Font_Weight_Custom_Control extends WP_Customize_Control { // phpcs:ignore
...
if ( ! class_exists( 'wi_Text_Transform_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for text-transform
	 *
	 * @deprecated since 1.3.40
	 */
	class wi_Text_Transform_Custom_Control extends WP_Customize_Control { // phpcs:ignore
...
if ( ! class_exists( 'wi_Customize_Slider_Control' ) ) {
	/**
	 * Create our container width slider control
	 *
	 * @deprecated 1.3.47
	 */
	class wi_Customize_Slider_Control extends WP_Customize_Control { // phpcs:ignore
		/**
		 * Render content.
		 */
		public function render_content() {}
