wp.customize.controlConstructor['witheme-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    ...
		    inputDefault,
		    changeAction,
			controlClass = '.customize-control-witheme-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.witheme-slider' ).each( function() {
		...
		jQuery( controlClass + ' .gp_range_value input[type=number]' ).on( 'input', function() {
			...
			jQuery( this ).closest( 'label' ).find( '.witheme-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .witheme-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				...
				slider_value = visible_area.find( '.witheme-slider' ),
		...
		jQuery( controlClass + ' .witheme-range-slider-control' ).each( function() {
			var _this = jQuery( this );
